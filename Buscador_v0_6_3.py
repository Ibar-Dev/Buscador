...
class MotorBusqueda:
    ...
    def cargar_excel_diccionario(self, ruta_str: str) -> bool:
        ...
        if not self._validar_columnas_df(
            self.datos_diccionario, self.indices_columnas_busqueda_dic, "diccionario"
        ):
            logger.warning(
                "Validación de columnas del diccionario fallida. Carga invalidada."
            )
            self.datos_diccionario = None
            self.archivo_diccionario_actual = None
            return False
    ...
    def _validar_columnas_df(
        self, df: Optional[pd.DataFrame], indices_cfg: List[int], nombre_df: str
    ) -> bool:
        ...
        if not indices_cfg or indices_cfg == [-1]:
            if num_cols_df == 0:
                logger.error(
                    f"El archivo del {nombre_df} está vacío o no contiene columnas (modo 'todas')."
                )
                messagebox.showerror(
                    f"Error de {nombre_df.capitalize()}",
                    f"El archivo del {nombre_df} está vacío o no contiene columnas.",
                )
                return False
            return True
        ...
        if not all(isinstance(idx, int) and idx >= 0 for idx in indices_cfg):
            logger.error(
                f"Configuración de índices para {nombre_df} inválida: {indices_cfg}. Deben ser enteros no negativos."
            )
            messagebox.showerror(
                "Error de Configuración",
                f"Los índices de columna para {nombre_df} deben ser números enteros no negativos. Config: {indices_cfg}",
            )
            return False
        ...
        if (
            num_cols_df == 0
        ):  # Esto ya lo cubre el primer if not indices_cfg, pero por si acaso.
            ...
            msg = (
                f"El {nombre_df} necesita al menos {max_indice_requerido + 1} columnas "
    ...
    def _obtener_nombres_columnas_busqueda_df(
        ...
        if df is None:
            logger.error(
                f"Intentando obtener nombres de columnas de un DataFrame ({nombre_df_log}) que es None."
            )
            return None
        ...
        if not indices_cfg or indices_cfg == [-1]:
            ...
            if cols_texto_obj:
                ...
                messagebox.showerror(
                    f"Error en {nombre_df_log.capitalize()}",
                    f"El archivo del {nombre_df_log} no tiene columnas.",
                )
                return None
    ...
    def _parsear_termino_busqueda_general(termino_raw: str) -> Tuple[str, List[str]]:
        ...
        termino_proc = (
            termino_raw.strip().upper()
        )  # Convertir a mayúsculas una vez aquí
        operador = "AND"
        ...
        logger.debug(
            f"Parseo general (interno): Operador='{operador}', Términos='{terminos_individuales}'"
        )
                    ...
                    item_analizado.update(
                        {
                            ...
                            "valor": v_num,
                            "unidad_busqueda": (
                                self.extractor_magnitud._normalizar_texto(
                                    unidad_str.strip()
                                )
                                if unidad_str
                                else None
    ...
    def _parse_numero(
        self, num_str: Any
    ) -> Optional[float]:  # Tu UI lo pasa como string, pero Any es más seguro
        # (Implementación de PyGuru)
        if not isinstance(num_str, (str, int, float)):
            return None
        try:
    ...
    def _generar_mascara_para_un_termino(
        ...
        for col_nombre in cols_a_buscar:
            if col_nombre not in df.columns:
                logger.warning(
                    f"Columna '{col_nombre}' no encontrada en el DataFrame al generar máscara. Saltando."
                )
                continue
            ...
            if tipo_sub in ["gt", "lt", "ge", "le", "range"]:
                for idx, valor_celda in col_series.items():
                    if pd.isna(valor_celda) or str(valor_celda).strip() == "":
                        continue
                    for match_celda in self.patron_num_unidad_df.finditer(
                        str(valor_celda)
                    ):
                        try:
                            num_celda_val = float(
                                match_celda.group(1).replace(",", ".")
                            )
                            unidad_celda_val = (
                                self.extractor_magnitud._normalizar_texto(
                                    match_celda.group(2)
                            ...
                            if (
                                unidad_sub_requerida
                                and unidad_celda_val != unidad_sub_requerida
                            ):
                                continue

                            cond_ok = False
                            if tipo_sub == "gt" and num_celda_val > valor_sub:
                                cond_ok = True
                            elif tipo_sub == "lt" and num_celda_val < valor_sub:
                                cond_ok = True
                            elif tipo_sub == "ge" and num_celda_val >= valor_sub:
                                cond_ok = True
                            elif tipo_sub == "le" and num_celda_val <= valor_sub:
                                cond_ok = True
                            elif (
                                tipo_sub == "range"
                                and valor_sub[0] <= num_celda_val <= valor_sub[1]
                            ):
                                cond_ok = True
                ...
                try:
                    if pd.api.types.is_string_dtype(
                        ...
                        mascara_col_actual = (
                            ...
                            .str.upper()
                            .str.contains(termino_regex_escapado, regex=True, na=False)
                        )
                    else:
                        mascara_col_actual = (
                            col_series.astype(str)
                    ...
                    logger.warning(
                        f"No se pudo convertir/buscar string en columna '{col_nombre}': {e_conv}"
                    )
    ...
    def _aplicar_mascara_combinada(
        self,
        df: pd.DataFrame,
        cols_a_buscar: List[str],
        ...
        if not cols_ok_en_df:
            logger.error(
                f"Ninguna columna de búsqueda ({cols_a_buscar}) existe en el DataFrame provisto."
            )
            return pd.Series(False, index=df.index)

        op_es_and = op_principal.upper() == "AND"
        mascara_final_combinada = (
        ...
        if (
            not terminos_analizados
        ):  # Si no hay términos válidos, el comportamiento depende del operador
        ...
        for termino_individual_analizado in terminos_analizados:
            mascara_este_termino = self._generar_mascara_para_un_termino(
                df, cols_ok_en_df, termino_individual_analizado
            )
            if op_es_and:
    ...
    def _extraer_terminos_diccionario(
        self, df_coincidencias: pd.DataFrame, cols_nombres: List[str]
    ) -> Set[str]:
        ...
        columnas_validas_en_df = [
            c for c in cols_nombres if c in df_coincidencias.columns
        ]
        if not columnas_validas_en_df:
            logger.warning(
                "Ninguna de las columnas configuradas para el diccionario existe en el DataFrame de coincidencias."
            )
            return terminos_clave
        ...
        for col_nombre in columnas_validas_en_df:
            try:
                nuevos_terminos = (
                    df_coincidencias[col_nombre]
                    .dropna()
                    .astype(str)
                    .str.upper()
                    .unique()
                )
                terminos_clave.update(
                    t for t in nuevos_terminos if t and not t.isspace()
                )
            except Exception as e:
                logger.warning(
                    f"Error extrayendo términos clave de la columna '{col_nombre}' del diccionario: {e}"
                )

        logger.info(
            f"Se extrajeron {len(terminos_clave)} términos clave del diccionario."
        )
        logger.debug(
            f"Términos clave extraídos (muestra): {list(terminos_clave)[:10]}..."
        )
        return terminos_clave

    def _buscar_terminos_en_descripciones(
        self,
        df_desc_a_buscar: pd.DataFrame,
        terminos_clave: Set[str],
        require_all_terminos_clave: bool = False,
    ) -> pd.DataFrame:
        # (Lógica de PyGuru _buscar_terminos_clave_en_descripciones, adaptada)
        if df_desc_a_buscar is None or df_desc_a_buscar.empty:
            logger.warning(
                "DataFrame de descripciones (para búsqueda de términos clave) no provisto o vacío."
            )
            return pd.DataFrame(
                columns=(
                    df_desc_a_buscar.columns if df_desc_a_buscar is not None else []
                )
            )
        ...
        if not terminos_clave:
            ...
            return pd.DataFrame(
                columns=df_desc_a_buscar.columns
            )  # Devolver estructura vacía

        logger.info(
            f"Buscando {len(terminos_clave)} términos clave en {len(df_desc_a_buscar)} descripciones (require_all={require_all_terminos_clave})."
        )

        texto_filas_desc = (
            df_desc_a_buscar.fillna("").astype(str).agg(" ".join, axis=1).str.upper()
        )
        ...
        if require_all_terminos_clave:
            try:
                mascara_final_desc = texto_filas_desc.apply(
                    lambda txt_fila: all(
                        re.search(pat, txt_fila, re.IGNORECASE)
                        for pat in terminos_escapados
                    )
                )
            except Exception as e:
                logger.exception(
                    f"Error durante la búsqueda 'require_all' en descripciones: {e}"
                )
                return pd.DataFrame(columns=df_desc_a_buscar.columns)
        else:
            patron_or_regex = "|".join(terminos_escapados)
            try:
                mascara_final_desc = texto_filas_desc.str.contains(
                    patron_or_regex, regex=True, case=False, na=False
                )
            except Exception as e:
                logger.exception(
                    f"Error durante la búsqueda 'OR' en descripciones: {e}"
                )
                return pd.DataFrame(columns=df_desc_a_buscar.columns)

        resultados_desc = df_desc_a_buscar[mascara_final_desc]
        logger.info(
            f"Búsqueda de términos clave en descripciones completada. Resultados: {len(resultados_desc)} filas."
        )
    ...
    def buscar(
        self,
        df_dic_para_busqueda: pd.DataFrame,
        df_desc_para_busqueda: pd.DataFrame,
        terminos_analizados_usuario: List[Dict[str, Any]],
        operador_principal_usuario: str,
        buscar_via_diccionario_flag: bool,
    ) -> Tuple[Optional[pd.DataFrame], OrigenResultados, Optional[pd.DataFrame]]:
        """
        ...
        if buscar_via_diccionario_flag:
            if df_dic_para_busqueda is None:
                return None, OrigenResultados.ERROR_CARGA_DICCIONARIO, None

            cols_dic = self._obtener_nombres_columnas_busqueda_df(
                df_dic_para_busqueda,
                self.indices_columnas_busqueda_dic,
                "diccionario (búsqueda)",
            )
            if not cols_dic:
                return None, OrigenResultados.ERROR_CONFIGURACION_COLUMNAS, None

            mascara_en_dic = self._aplicar_mascara_combinada(
                df_dic_para_busqueda,
                cols_dic,
                terminos_analizados_usuario,
                operador_principal_usuario,
            )
            ...
            if fcds_obtenidos.empty:
                return (
                    pd.DataFrame(
                        columns=(
                            df_desc_para_busqueda.columns
                            if df_desc_para_busqueda is not None
                            else []
                        )
                    ),
                    OrigenResultados.VIA_DICCIONARIO_SIN_RESULTADOS_DESC,
            ...
            terminos_extraidos_de_fcds = self._extraer_terminos_diccionario(
                fcds_obtenidos, cols_dic
            )
            ...
            if df_desc_para_busqueda is None:
                return None, OrigenResultados.ERROR_CARGA_DESCRIPCION, fcds_obtenidos
            ...
            resultados_desc_via_dic = self._buscar_terminos_en_descripciones(
                df_desc_para_busqueda,
                terminos_extraidos_de_fcds,
                require_all_terminos_clave=False,
            )

            if resultados_desc_via_dic.empty:
                return (
                    pd.DataFrame(columns=df_desc_para_busqueda.columns),
                    OrigenResultados.VIA_DICCIONARIO_SIN_RESULTADOS_DESC,
                    fcds_obtenidos,
                )
            else:
                return (
                    resultados_desc_via_dic,
                    OrigenResultados.VIA_DICCIONARIO_CON_RESULTADOS_DESC,
                    fcds_obtenidos,
                )
        ...
        # Búsqueda directa en descripciones
            ...
            if df_desc_para_busqueda is None:
                return None, OrigenResultados.ERROR_CARGA_DESCRIPCION, None

            # Para búsqueda directa, usar todas las columnas del df de descripciones
            cols_desc = self._obtener_nombres_columnas_busqueda_df(
                df_desc_para_busqueda, [], "descripciones (búsqueda directa)"
            )
            if not cols_desc:
                return (
                    None,
                    OrigenResultados.ERROR_CONFIGURACION_COLUMNAS,
                    None,
                )  # No debería pasar si el df tiene columnas
            ...
            logger.info(
                f"Resultados directos en descripción: {len(resultados_directos)}"
            )
...
class InterfazGrafica(tk.Tk):
    ...
    def __init__(self):
        ...
        indices_cfg = self.config.get(
            "indices_columnas_busqueda_dic", []
        )  # Default a [] (todas texto)
        ...
        self.df_candidato_descripcion: Optional[pd.DataFrame] = (
            None  # Resultados finales de descripción
        )
    ...
    def _cargar_configuracion(self) -> Dict:
        ...
        if os.path.exists(self.CONFIG_FILE):
            try:
                ...
                messagebox.showwarning(
                    "Error Configuración", f"No se pudo cargar config:\n{e}"
                )
        else:
        ...
        config["last_dic_path"] = (
            str(Path(last_dic_path_str)) if last_dic_path_str else None
        )

        last_desc_path_str = config.get("last_desc_path")
        config["last_desc_path"] = (
            str(Path(last_desc_path_str)) if last_desc_path_str else None
        )
    ...
    def _guardar_configuracion(self):
        ...
        self.config["last_dic_path"] = (
            str(self.motor.archivo_diccionario_actual)
            if self.motor.archivo_diccionario_actual
            else None
        )
        self.config["last_desc_path"] = (
            str(self.motor.archivo_descripcion_actual)
        ...
        try:
            ...
            messagebox.showerror(
                "Error Configuración", f"No se pudo guardar config:\n{e}"
            )
    ...
    def _configurar_estilo_ttk(self):
        ...
        if theme_to_use:
            ...
            try:
                ...
                # Configurar estilo personalizado para los botones operadores
                style.configure(
                    "Operator.TButton",
                    padding=(2, 1),  # Padding reducido pero equilibrado
                    width=2,  # Ancho fijo pequeño
                    font=('TkDefaultFont', 9)  # Fuente más pequeña
                )
            except tk.TclError as e:
                logger.warning(f"No se pudo aplicar tema '{theme_to_use}': {e}.")
        else:
            logger.warning("No se encontró tema TTK disponible.")

    def _crear_widgets(self):
        self.marco_controles = ttk.LabelFrame(self, text="Controles")

        # Configurar las columnas del marco_controles para que se expandan correctamente
        for i in range(6):  # Configurar 6 columnas para los controles
            self.marco_controles.grid_columnconfigure(i, weight=1)
        ...
        # Frame para botones operadores
        self.frame_ops = ttk.Frame(self.marco_controles)
        
        # Creación de botones operadores
        self.btn_and = ttk.Button(
            self.frame_ops,
            text="+",
            command=lambda: self._insertar_operador_validado("+"),
            style="Operator.TButton",
            width=2
        )
        self.btn_or = ttk.Button(
            self.frame_ops,
            text="|",
            command=lambda: self._insertar_operador_validado("|"),
            style="Operator.TButton",
        ...
        # Configuración del grid para los botones con menos espacio
        button_grid_config = {"padx": 1, "pady": 1, "sticky": "nsew"}
        self.btn_and.grid(row=0, column=0, **button_grid_config)
        self.btn_or.grid(row=0, column=1, **button_grid_config)
        self.btn_not.grid(row=0, column=2, **button_grid_config)
        self.btn_gt.grid(row=0, column=3, **button_grid_config)
        self.btn_lt.grid(row=0, column=4, **button_grid_config)
        self.btn_ge.grid(row=0, column=5, **button_grid_config)
        self.btn_le.grid(row=0, column=6, **button_grid_config)
        self.btn_range.grid(row=0, column=7, **button_grid_config)

        self.entrada_busqueda = ttk.Entry(
            self.marco_controles, width=50, textvariable=self.texto_busqueda_var
        )
        self.btn_buscar = ttk.Button(
            self.marco_controles, text="Buscar", command=self._ejecutar_busqueda
        )  # No necesita lambda aquí
        self.btn_salvar_regla = ttk.Button(
            self.marco_controles, text="Salvar Regla", command=self._salvar_regla_actual
        )
        self.btn_ayuda = ttk.Button(
            self.marco_controles, text="?", command=self._mostrar_ayuda, width=3
        )
        self.btn_exportar = ttk.Button(
            self.marco_controles, text="Exportar", command=self._exportar_resultados
        )
        ...
        self.tabla_diccionario = ttk.Treeview(
            self.frame_tabla_diccionario, show="headings", height=8
        )
        self.scrolly_diccionario = ttk.Scrollbar(
            self.frame_tabla_diccionario,
            orient="vertical",
            command=self.tabla_diccionario.yview,
        )
        self.scrollx_diccionario = ttk.Scrollbar(
            self.frame_tabla_diccionario,
            orient="horizontal",
            command=self.tabla_diccionario.xview,
        )
        self.tabla_diccionario.configure(
            yscrollcommand=self.scrolly_diccionario.set,
            xscrollcommand=self.scrollx_diccionario.set,
        )

        self.frame_tabla_resultados = ttk.Frame(self)
        self.tabla_resultados = ttk.Treeview(
            self.frame_tabla_resultados, show="headings"
        )
        self.scrolly_resultados = ttk.Scrollbar(
            self.frame_tabla_resultados,
            orient="vertical",
        ...
        self.tabla_resultados.configure(
            yscrollcommand=self.scrolly_resultados.set,
            xscrollcommand=self.scrollx_resultados.set,
        )

        self.barra_estado = ttk.Label(
            self, text="", relief=tk.SUNKEN, anchor=tk.W, borderwidth=1
        )
    ...
    def _configurar_grid(self):
        ...
        # Configurar todas las columnas del marco_controles
        for i in range(6):
            self.marco_controles.grid_columnconfigure(i, weight=1)
        # Dar más peso a las columnas que contienen las etiquetas
        self.marco_controles.grid_columnconfigure(1, weight=2)
        self.marco_controles.grid_columnconfigure(3, weight=2)

        self.btn_cargar_diccionario.grid(
            row=0, column=0, padx=(5, 0), pady=5, sticky="w"
        )
        self.lbl_dic_cargado.grid(row=0, column=1, padx=(2, 10), pady=5, sticky="ew")
        self.btn_cargar_descripciones.grid(
            row=0, column=2, padx=(5, 0), pady=5, sticky="w"
        )
        self.lbl_desc_cargado.grid(row=0, column=3, padx=(2, 5), pady=5, sticky="ew")

        # Configurar frame_ops para que ocupe todo el ancho y los botones se distribuyan uniformemente
        self.frame_ops.grid(
            row=1, column=0, columnspan=6, padx=5, pady=(5, 0), sticky="ew"
        )

        # Configurar las columnas del frame_ops para que los botones se distribuyan uniformemente
        for i in range(8):  # Para los 8 botones operadores
        ...
        self.btn_salvar_regla.grid(
            row=2, column=3, padx=(2, 0), pady=(0, 5), sticky="w"
        )
        ...
        self.lbl_tabla_diccionario.grid(
            row=1, column=0, sticky="sw", padx=10, pady=(10, 0)
        )
        self.lbl_tabla_resultados.grid(
            row=3, column=0, sticky="sw", padx=10, pady=(0, 0)
        )
        self.frame_tabla_diccionario.grid(
        ...
        self.frame_tabla_resultados.grid(
            row=4, column=0, sticky="nsew", padx=10, pady=(0, 10)
        )
...
"""
    ...
    def _configurar_tags_treeview(self):
        ...
        self.tabla_diccionario.tag_configure("resalto", background="#ffeb99")
    ...
    def _ordenar_columna(self, tabla: ttk.Treeview, col: str, reverse: bool):
        ...
        if (
            df_para_ordenar is None
            or df_para_ordenar.empty
            or col not in df_para_ordenar.columns
        ):
            logging.debug(
                f"No se puede ordenar la tabla por columna '{col}'. Datos no disponibles o columna inexistente."
            )
            tabla.heading(
                col,
                command=lambda c=col, t=tabla: self._ordenar_columna(t, c, not reverse),
            )  # Invertir para siguiente click
            return
        ...
        try:
            ...
            if pd.api.types.is_numeric_dtype(
                df_para_ordenar[col].infer_objects().dtype
            ):
            ...
            if tabla == self.tabla_diccionario:
                self.motor.datos_diccionario = (
                    df_ordenado  # Actualiza el DataFrame del motor
                )
                self._actualizar_tabla(
                    tabla, df_ordenado, limite_filas=100
                )  # Mostrar preview actualizada
            elif tabla == self.tabla_resultados:
            ...
            self._actualizar_estado(
                f"Tabla ordenada por '{col}' ({'Asc' if not reverse else 'Desc'})."
            )
        except Exception as e:
            logging.exception(f"Error al intentar ordenar por columna '{col}'")
            messagebox.showerror(
                "Error al Ordenar", f"No se pudo ordenar por '{col}':\n{e}"
            )
            tabla.heading(
                col, command=lambda c=col, t=tabla: self._ordenar_columna(t, c, False)
            )  # Reset
    ...
    def _actualizar_tabla(
        ...
        columnas_a_mostrar: Optional[List[str]] = None,
    ):
        # (Lógica de tu UI, con pequeñas adaptaciones)
        is_diccionario = tabla == self.tabla_diccionario
        logger.debug(
            f"Actualizando tabla {'Diccionario' if is_diccionario else 'Resultados'}."
        )
        try:
            for i in tabla.get_children():
                tabla.delete(i)
        except tk.TclError as e:
            logger.warning(f"Error Tcl al limpiar tabla: {e}")
            pass
        tabla["columns"] = ()
        ...
        if columnas_a_mostrar:
            ...
            if not cols_finales:
                cols_finales = (
                    cols_df  # Fallback a todas si ninguna de las especificadas existe
                )
        else:
        ...
        if not cols_finales:
            logger.warning(
                "DataFrame no tiene columnas para mostrar o columnas seleccionadas no existen."
            )
            return
        ...
        for col in cols_finales:
            ...
            try:
                ...
                ancho_contenido = (
                    col_as_str.str.len().max() if not col_as_str.empty else 0
                )
                ancho_cabecera = len(str(col))
                ancho = max(
                    70,
                    min(int(max(ancho_cabecera * 8, ancho_contenido * 6.5) + 25), 400),
                )
                tabla.column(col, anchor=tk.W, width=ancho, minwidth=70)
            except Exception as e:
                logger.warning(
                    f"Error calculando ancho para col '{col}': {e}. Ancho default."
                )
                tabla.column(col, anchor=tk.W, width=100, minwidth=50)

        num_filas_a_iterar = (
            limite_filas
            if is_diccionario and limite_filas is not None
            else len(df_para_mostrar_vista)
        )
        ...
        for i, (_, row) in enumerate(df_iterar.iterrows()):
            ...
            try:
                ...
                logger.warning(
                    f"Error Tcl insertando fila {i}: {e}. Intentando con ASCII."
                )
                try:
                    vals_ascii = [
                        v.encode("ascii", "ignore").decode("ascii") for v in vals
                    ]
    ...
    def _actualizar_etiquetas_archivos(self):
        ...
        dic_display = (
            f"Dic: {dic_name}"
            if len(dic_name) <= max_len_label
            else f"Dic: ...{dic_name[-(max_len_label-4):]}"
        )
        desc_display = (
            f"Desc: {desc_name}"
            if len(desc_name) <= max_len_label
            else f"Desc: ...{desc_name[-(max_len_label-4):]}"
        )

        self.lbl_dic_cargado.config(
            text=dic_display, foreground="green" if dic_path else "red"
        )
        self.lbl_desc_cargado.config(
            text=desc_display, foreground="green" if desc_path else "red"
        )

    def _actualizar_botones_estado_general(self):
        # (Tu lógica, adaptada ligeramente)
        dic_cargado = (
            self.motor.datos_diccionario is not None
        )  # and not self.motor.datos_diccionario.empty (puede estar vacío y ser válido)
        desc_cargado = (
            self.motor.datos_descripcion is not None
        )  # and not self.motor.datos_descripcion.empty
        ...
        self.btn_buscar["state"] = (
            "normal" if dic_cargado and desc_cargado else "disabled"
        )
        ...
        if (
            ...
            if self.origen_principal_resultados.es_via_diccionario:
                if (
                    self.df_candidato_diccionario is not None
                    and not self.df_candidato_diccionario.empty
                ) or (
                    self.df_candidato_descripcion is not None
                    and not self.df_candidato_descripcion.empty
                    and self.origen_principal_resultados
                    == OrigenResultados.VIA_DICCIONARIO_CON_RESULTADOS_DESC
                ):
                    puede_salvar_algo = True
            elif (
                self.origen_principal_resultados.es_directo_descripcion
                or self.origen_principal_resultados
                == OrigenResultados.DIRECTO_DESCRIPCION_VACIA
            ):
                if (
                    self.df_candidato_descripcion is not None
                ):  # Puede estar vacío si se guardó "todo"
                    puede_salvar_algo = True
    ...
    def _cargar_diccionario(self):
        ...
        last_dir = (
            str(Path(last_dir_str).parent)
            if last_dir_str and Path(last_dir_str).exists()
            else os.getcwd()
        )
        ...
        if not ruta:
            logger.info("Carga de diccionario cancelada.")
            return
        ...
        if self.motor.cargar_excel_diccionario(ruta):
            ...
            if df_dic is not None:
                ...
                cols_busqueda_nombres = (
                    self.motor._obtener_nombres_columnas_busqueda_df(
                        df_dic,
                        self.motor.indices_columnas_busqueda_dic,
                        "diccionario (preview)",
                    )
                ...
                indices_str = (
                    ", ".join(map(str, self.motor.indices_columnas_busqueda_dic))
                    if self.motor.indices_columnas_busqueda_dic
                    and self.motor.indices_columnas_busqueda_dic != [-1]
                    else "Todas Texto"
                )
                ...
                self._actualizar_tabla(
                    ...
                    df_dic,
                    limite_filas=100,
                    columnas_a_mostrar=cols_busqueda_nombres,
                )
                self.title(f"Buscador - Dic: {nombre_archivo}")
                self._actualizar_estado(
                    f"Diccionario '{nombre_archivo}' ({num_filas} filas) cargado."
                )
        else:
    ...
    def _cargar_excel_descripcion(self):
        ...
        if not ruta:
            logger.info("Carga de descripciones cancelada.")
            return
        ...
        if self.motor.cargar_excel_descripcion(ruta):
            ...
            if df_desc is not None:
                ...
                self._actualizar_estado(
                    f"Descripciones '{nombre_archivo}' ({num_filas} filas) cargadas. Mostrando datos..."
                )
                self._actualizar_tabla(self.tabla_resultados, df_desc)
                dic_n_title = (
                    Path(self.motor.archivo_diccionario_actual).name
                    if self.motor.archivo_diccionario_actual
                    else "N/A"
                )
    ...
    def _parsear_termino_busqueda_inicial(
        self, termino_raw: str
    ) -> Tuple[str, List[str]]:
        ...
        if "+" in termino_limpio:
            op_principal = "AND"
            terminos_brutos = [
                p.strip() for p in termino_limpio.split("+") if p.strip()
            ]
        ...
        if not any(terminos_brutos):
            logger.warning(f"Término '{termino_raw}' vacío tras parseo inicial.")
            return "OR", []
    ...
    def _ejecutar_busqueda(self):
        if self.motor.datos_diccionario is None or self.motor.datos_descripcion is None:
            messagebox.showwarning(
                "Archivos Faltantes", "Cargue Diccionario y Descripciones."
            )
            return
        ...
        if not termino_busqueda_actual.strip():
            ...
            self.resultados_actuales = (
                df_desc_all.copy() if df_desc_all is not None else pd.DataFrame()
            )
            self.df_candidato_descripcion = self.resultados_actuales
            self.origen_principal_resultados = (
                OrigenResultados.DIRECTO_DESCRIPCION_VACIA
            )  # Indica que se mostraron todas
            self._actualizar_estado(
                f"Mostrando todas las {len(df_desc_all) if df_desc_all is not None else 0} descripciones."
            )
            self._actualizar_botones_estado_general()
            return
        ...
        df_dic_copia = (
            self.motor.datos_diccionario.copy()
            if self.motor.datos_diccionario is not None
            else None
        )
        df_desc_copia = (
            self.motor.datos_descripcion.copy()
        ...
        if df_dic_copia is None or df_desc_copia is None:  # Doble check
            messagebox.showerror(
                "Error Interno",
                "Los DataFrames base no están disponibles para búsqueda.",
            )
            self._actualizar_estado("Error interno en DataFrames.")
            return

        op_principal_usuario, terminos_brutos_usuario = (
            self._parsear_termino_busqueda_inicial(termino_busqueda_actual)
        )
        if not terminos_brutos_usuario:
            messagebox.showwarning(
                "Término Inválido", "El término de búsqueda está vacío o malformado."
            )
            self._actualizar_estado("Término de búsqueda inválido.")
            self._actualizar_botones_estado_general()
            return

        terminos_analizados_usuario = self.motor._analizar_terminos(
            terminos_brutos_usuario
        )
        if not terminos_analizados_usuario:
            messagebox.showwarning(
                "Término Inválido",
                f"No se pudieron analizar los términos en '{termino_busqueda_actual}'. Verifique sintaxis.",
            )
            self._actualizar_estado(f"Análisis de '{termino_busqueda_actual}' fallido.")
            self._actualizar_botones_estado_general()
            return
        ...
        if origen_res == OrigenResultados.VIA_DICCIONARIO_CON_RESULTADOS_DESC:
            ...
            self.df_candidato_descripcion = (
                resultados_busqueda.copy()
                if resultados_busqueda is not None
                else pd.DataFrame()
            )
            self._actualizar_estado(
                f"'{termino_busqueda_actual}': {len(fcds) if fcds is not None else 0} en Dic, {len(resultados_busqueda) if resultados_busqueda is not None else 0} en Desc."
            )
        elif origen_res == OrigenResultados.VIA_DICCIONARIO_SIN_RESULTADOS_DESC:
            self.resultados_actuales = pd.DataFrame(
                columns=df_desc_copia.columns
            )  # Vacío pero con estructura
            self.df_candidato_descripcion = self.resultados_actuales.copy()
            self._actualizar_estado(
                f"'{termino_busqueda_actual}': {len(fcds) if fcds is not None else 0} en Dic, 0 en Desc."
            )
            messagebox.showinfo(
                "Información",
                f"Se encontraron {len(fcds) if fcds is not None else 0} filas en Diccionario para '{termino_busqueda_actual}', pero ninguna coincidencia en Descripciones.",
            )
            ...
            self.df_candidato_descripcion = self.resultados_actuales.copy()
            self._actualizar_estado(
                f"'{termino_busqueda_actual}': {len(fcds) if fcds is not None else 0} en Dic, sin términos válidos para Desc."
            )
            messagebox.showinfo(
                "Información",
                f"Se encontraron {len(fcds) if fcds is not None else 0} filas en Diccionario, pero no se extrajeron términos válidos para buscar en descripciones.",
            )
        ...
        if (
            origen_res.es_via_diccionario
            and (resultados_busqueda is None or resultados_busqueda.empty)
        ) or (
            fcds is None
            or fcds.empty
            and origen_res == OrigenResultados.VIA_DICCIONARIO_SIN_RESULTADOS_DESC
        ):  # Si no hubo FCDs

            mensaje_fallback = f"'{termino_busqueda_actual}' no produjo resultados finales vía diccionario."
            if fcds is None or fcds.empty:
                mensaje_fallback = (
                    f"'{termino_busqueda_actual}' no se encontró en el Diccionario."
                )
            ...
            if messagebox.askyesno(
                ...
                self.df_candidato_descripcion = (
                    resultados_directos.copy()
                    if resultados_directos is not None
                    else pd.DataFrame()
                )
                self.origen_principal_resultados = (
                    origen_directo  # Actualizar origen principal
                )
                self.df_candidato_diccionario = (
                    None  # Ya no aplica si la búsqueda fue directa
                )

                num_rdd = (
                    len(self.resultados_actuales)
                    if self.resultados_actuales is not None
                    else 0
                )
                self._actualizar_estado(
                    f"Búsqueda directa '{termino_busqueda_actual}': {num_rdd} resultados."
                )
                if num_rdd == 0:
                    messagebox.showinfo(
                        "Información",
                        f"No se encontraron resultados para '{termino_busqueda_actual}' en búsqueda directa.",
                    )
            else:
                ...
                if self.resultados_actuales is None or self.resultados_actuales.empty:
                    self._actualizar_estado(
                        f"Búsqueda de '{termino_busqueda_actual}' cancelada/sin resultados."
                    )
                    self.resultados_actuales = pd.DataFrame(
                        columns=df_desc_copia.columns
                    )  # Mostrar tabla vacía
                    self.df_candidato_descripcion = self.resultados_actuales.copy()
                    self.origen_principal_resultados = (
                        OrigenResultados.NINGUNO
                    )  # O el último origen de error/sin_resultados
        ...
        if (
            self.motor.datos_diccionario is not None
            and not self.motor.datos_diccionario.empty
        ):
            self._buscar_y_enfocar_en_preview()

    def _buscar_y_enfocar_en_preview(self):
        # (Lógica de tu UI, con pequeñas adaptaciones)
        df_completo = self.motor.datos_diccionario
        if df_completo is None or df_completo.empty:
            return

        termino_buscar_raw = self.texto_busqueda_var.get()
        if not termino_buscar_raw:
            return

        # Get the initial operator and raw terms - unused operator is fine since this is preview-only
        _, terminos_brutos = self._parsear_termino_busqueda_inicial(termino_buscar_raw)

        # Remove special operator symbols from terms for preview focusing
        clean_terminos = []
        for t in terminos_brutos:
            t = t.strip()
            if t.startswith("#"):  # Remove negation
                t = t[1:].strip()
            ...
            for op in [">=", "<=", ">", "<"]:
                if t.startswith(op):
                    ...
                    break
            if t:
                clean_terminos.append(t)

        terminos_brutos = clean_terminos
        ...
        if analizado_enfocar_lista:
            ...
            if isinstance(val_analizado, (float, int)):
                ...
                termino_enfocar_valor_str = (
                    termino_enfocar.upper()
                )  # Asegurar upper para comparación
        else:
            termino_enfocar_valor_str = termino_enfocar.upper()

        if not termino_enfocar_valor_str:
            return

        items_preview = self.tabla_diccionario.get_children("")
        if not items_preview or self.motor.datos_diccionario is None:
            return

        termino_upper_para_treeview = (
            termino_enfocar_valor_str  # Ya debería estar en upper
        )
        logger.info(
            f"Intentando enfocar '{termino_upper_para_treeview}' en vista previa del diccionario..."
        )
        ...
        for item_id in items_preview:
            try:
                ...
                if any(
                    termino_upper_para_treeview in str(val).upper()
                ...
                logger.warning(
                    f"Error procesando item {item_id} en preview (búsqueda treeview): {e}"
                )
                continue
        ...
        if found_item_id:
            logger.info(
                f"Término '{termino_upper_para_treeview}' enfocado en preview (item ID: {found_item_id})."
            )
            try:
            ...
            logger.info(
                f"Término '{termino_upper_para_treeview}' no encontrado/enfocado en vista previa del diccionario."
            )
    ...
    def _salvar_regla_actual(self):
        ...
        logger.info(
            f"Intentando salvar regla. Origen: {origen_nombre_actual}, Último término: '{self.ultimo_termino_buscado}'"
        )
        ...
        op_regla, terminos_brutos_regla = self._parsear_termino_busqueda_inicial(
            termino_original_regla
        )
        ...
        if self.origen_principal_resultados.es_via_diccionario:
            ...
            if decision["confirmed"]:
                if (
                    decision["save_fcd"]
                    and self.df_candidato_diccionario is not None
                    and not self.df_candidato_diccionario.empty
                ):
                    regla_fcd = {
                        **regla_a_guardar_base,
                        "tipo_datos_guardados": "COINCIDENCIAS_DICCIONARIO",
                        "datos_snapshot": self.df_candidato_diccionario.to_dict(
                            orient="records"
                        ),
                    }
                    self.reglas_guardadas.append(regla_fcd)
                    salvo_algo = True
                    logger.info(f"Regla (FCD) salvada para: '{termino_original_regla}'")
                if (
                    decision["save_rfd"]
                    and self.df_candidato_descripcion is not None
                    and not self.df_candidato_descripcion.empty
                    and self.origen_principal_resultados
                    == OrigenResultados.VIA_DICCIONARIO_CON_RESULTADOS_DESC
                ):
                    regla_rfd = {
                        **regla_a_guardar_base,
                        "tipo_datos_guardados": "RESULTADOS_DESCRIPCION_VIA_DICCIONARIO",
                        "datos_snapshot": self.df_candidato_descripcion.to_dict(
                            orient="records"
                        ),
                    }
                    self.reglas_guardadas.append(regla_rfd)
                    salvo_algo = True
                    logger.info(
                        f"Regla (Resultados Desc vía Dic) salvada para: '{termino_original_regla}'"
                    )
        elif (
            self.origen_principal_resultados.es_directo_descripcion
            or self.origen_principal_resultados
            ...
            if (
                self.df_candidato_descripcion is not None
            ):  # Puede ser un DF vacío si se guardó "todo"
                tipo_datos = (
                    "TODAS_LAS_DESCRIPCIONES"
                    if self.origen_principal_resultados
                    == OrigenResultados.DIRECTO_DESCRIPCION_VACIA
                    else "RESULTADOS_DESCRIPCION_DIRECTA"
                )
                regla_directa = {
                    **regla_a_guardar_base,
                    "tipo_datos_guardados": tipo_datos,
                    "datos_snapshot": self.df_candidato_descripcion.to_dict(
                        orient="records"
                    ),
                }
                self.reglas_guardadas.append(regla_directa)
                salvo_algo = True
                logger.info(
                    f"Regla ({tipo_datos}) salvada para: '{termino_original_regla}'"
                )
            else:
                messagebox.showwarning("Sin Datos", "No hay resultados para salvar.")
        else:
            if self.origen_principal_resultados != OrigenResultados.NINGUNO:
                messagebox.showerror(
                    "Error",
                    f"No se puede determinar qué salvar para origen: {origen_nombre_actual}.",
                )

        if salvo_algo:
            self._actualizar_estado(
                f"Regla(s) nueva(s) guardada(s). Total: {len(self.reglas_guardadas)}."
            )
        else:
            self._actualizar_estado("Ninguna regla fue salvada.")
        self._actualizar_botones_estado_general()
    ...
    def _mostrar_dialogo_seleccion_salvado_via_diccionario(self) -> Dict[str, bool]:
        ...
        var_fcd = tk.BooleanVar(
            value=(
                True
                if self.df_candidato_diccionario is not None
                and not self.df_candidato_diccionario.empty
                else False
            )
        )
        var_rfd = tk.BooleanVar(
            value=(
                True
                if self.df_candidato_descripcion is not None
                and not self.df_candidato_descripcion.empty
                and self.origen_principal_resultados
                == OrigenResultados.VIA_DICCIONARIO_CON_RESULTADOS_DESC
                else False
        ...
        lbl_msg = ttk.Label(
            dialog,
            text="La búsqueda fue vía Diccionario. ¿Qué datos desea salvar para esta regla?",
        )
        lbl_msg.pack(pady=10, padx=10)

        chk_fcd = ttk.Checkbutton(
            dialog, text="Coincidencias del Diccionario (FCDs)", variable=var_fcd
        )
        ...
        chk_rfd = ttk.Checkbutton(
            dialog, text="Resultados Finales de Descripciones (RFDs)", variable=var_rfd
        )
        ...
        def on_confirm():
            ...
            if not decision["save_fcd"] and not decision["save_rfd"]:
                messagebox.showwarning(
                    "Nada Seleccionado",
                    "No ha seleccionado ningún conjunto de datos para salvar.",
                    parent=dialog,
                )
        ...
        btn_confirm = ttk.Button(
            frame_botones_dialogo, text="Confirmar", command=on_confirm
        )
        btn_confirm.pack(side=tk.LEFT, padx=10)
        btn_cancel = ttk.Button(
            frame_botones_dialogo, text="Cancelar", command=on_cancel
        )
    ...
    def _exportar_resultados(self):
        ...
        if not self.reglas_guardadas:
            messagebox.showwarning(
                "Sin Reglas", "No hay reglas guardadas para exportar."
            )
            return
        ...
        self._actualizar_estado("Exportando reglas...")
        num_reglas = len(self.reglas_guardadas)
            ...
            with pd.ExcelWriter(ruta_guardar, engine="openpyxl") as writer:
                ...
                for i, regla_guardada in enumerate(self.reglas_guardadas):
                    ...
                    datos_indice_export.append(
                        {
                            ...
                            "Termino_Busqueda_Original": regla_guardada.get(
                                "termino_busqueda_original", "N/A"
                            ),
                            "Operador_Principal": regla_guardada.get(
                                "operador_principal", "N/A"
                            ),
                            "Tipo_Datos_Guardados": regla_guardada.get(
                                "tipo_datos_guardados", "N/A"
                            ),
                            "Fuente_Original_Resultados": regla_guardada.get(
                                "fuente_original_guardado", "N/A"
                            ),
                            "Timestamp_Guardado": regla_guardada.get(
                                "timestamp", "N/A"
                            ),
                            "Num_Filas_Snapshot": len(
                                regla_guardada.get("datos_snapshot", [])
                            ),
                        }
                    )
                    ...
                    df_regla_definicion = pd.DataFrame(
                        [
                            {
                                "termino_original": regla_guardada.get(
                                    "termino_busqueda_original"
                                ),
                                "operador_principal": regla_guardada.get(
                                    "operador_principal"
                                ),
                                "terminos_analizados_json": json.dumps(
                                    regla_guardada.get("termino_busqueda_parseado"),
                                    ensure_ascii=False,
                                    indent=2,
                                ),
                            }
                    ...
                    df_regla_definicion.to_excel(
                        writer, sheet_name=nombre_hoja_def, index=False
                    )
                    ...
                    if datos_snapshot_list:
                        ...
                        if not df_snapshot.empty:
                            ...
                            df_snapshot.to_excel(
                                writer, sheet_name=nombre_hoja_snap, index=False
                            )

                if datos_indice_export:
                    pd.DataFrame(datos_indice_export).to_excel(
                        writer, sheet_name="Indice_Reglas_Exportadas", index=False
                    )

            logging.info(
                f"Exportación de {num_reglas} regla(s) completada a {ruta_guardar}"
            )
            messagebox.showinfo(
                "Exportación Exitosa",
                f"{num_reglas} regla(s) exportadas a:\n{ruta_guardar}",
            )
    ...
    def _actualizar_estado_botones_operadores(self):
        ...
        # Encontrar inicio del término actual
        segmento_antes_cursor = texto_completo[:cursor_pos]
        ultimo_op_logico_pos = -1
        for op_s in ["+", "|", "/"]:
            pos = segmento_antes_cursor.rfind(op_s)
            if pos > ultimo_op_logico_pos:
                ultimo_op_logico_pos = pos

        # El término actual es desde el último operador lógico (o inicio) hasta el cursor
        termino_actual_bruto = segmento_antes_cursor[
            ultimo_op_logico_pos + 1 :
        ].lstrip()

        # Analizar este término para ver qué contiene
        terminos_analizados_actual = self.motor._analizar_terminos(
            [termino_actual_bruto]
        )
        tipo_termino_actual = None
        tiene_negacion_actual = False

        if terminos_analizados_actual:
            analisis_primero = terminos_analizados_actual[0]
            tipo_termino_actual = analisis_primero.get("tipo")
            if analisis_primero.get("negate"):
                tiene_negacion_actual = True

        # Botón NOT (#): habilitar solo si estamos al inicio de un término nuevo
        estado_not = "normal"
        if tiene_negacion_actual or termino_actual_bruto.strip():
            estado_not = "disabled"
        self.btn_not["state"] = estado_not

        # Operadores de comparación y rango
        es_comparativo_o_rango_actual = tipo_termino_actual in [
            "gt",
            "lt",
            "ge",
            "le",
        ...
        estado_comparacion = (
            "disabled"
            if es_comparativo_o_rango_actual
            or (tiene_negacion_actual and not termino_actual_bruto[1:].strip())
            else "normal"
        )
        for btn in [self.btn_gt, self.btn_lt, self.btn_ge, self.btn_le]:
            btn.config(state=estado_comparacion)

        # Rango: necesita algo antes y no puede haber otros operadores
        puede_poner_rango = False
        if (
            termino_actual_bruto.strip()
            and not tiene_negacion_actual
            and not es_comparativo_o_rango_actual
        ):
            if termino_actual_bruto[-1].isdigit() or termino_actual_bruto[-1].isalpha():
                puede_poner_rango = True
        self.btn_range.config(state="normal" if puede_poner_rango else "disabled")

        # Operadores lógicos: deshabilitados si el texto está vacío o termina en operador
        estado_logico = "normal"
        texto_limpio_final_total = texto_completo.rstrip()
        if not texto_completo.strip():
            estado_logico = "disabled"
        elif texto_limpio_final_total and texto_limpio_final_total[-1] in [
            "+",
            "|",
            "/",
        ]:
            estado_logico = "disabled"
        elif not termino_actual_bruto.strip() and ultimo_op_logico_pos != -1:
            estado_logico = "disabled"
    ...
    def _insertar_operador_validado(self, operador: str):
        ...
        # Obtener el segmento actual y la posición relativa al inicio del segmento
        inicio_segmento = 0
        temp_pos = texto_actual.rfind("+", 0, cursor_pos)
        if temp_pos != -1:
            inicio_segmento = temp_pos + 1
        temp_pos = texto_actual.rfind("|", 0, cursor_pos)
        if temp_pos != -1:
            inicio_segmento = max(inicio_segmento, temp_pos + 1)
        temp_pos = texto_actual.rfind("/", 0, cursor_pos)
        if temp_pos != -1:
            inicio_segmento = max(inicio_segmento, temp_pos + 1)

        segmento_actual = texto_actual[inicio_segmento:cursor_pos].lstrip()
        puede_insertar = True

        # Validar según el tipo de operador
        if operador in ["+", "|", "/"]:
            if not texto_actual.strip() or texto_actual[
                cursor_pos - 1 : cursor_pos
            ] in ["+", "|", "/", " "]:
                puede_insertar = False
            # Insertar con espacios para legibilidad
            operador = f" {operador} "
        elif operador == "#":
            # Solo permitir al inicio del término lógico (después de operador lógico o inicio)
            # y solo si no hay otros operadores ya presentes
            if segmento_actual or (
                cursor_pos > inicio_segmento
                and texto_actual[inicio_segmento:cursor_pos].strip()
            ):
                puede_insertar = False
        elif operador in [">", "<", ">=", "<=", "-"]:
            # Lógica para operadores de comparación y rango
            puede_insertar = self.btn_gt["state"] == "normal"
...
if __name__ == "__main__":
    ...
    try:
        ...
        missing_deps.append("openpyxl (para .xlsx)")
        # No es crítico si solo usas .xls
    ...
    try:
        ...
        try:
            ...
            root_err.withdraw()
            messagebox.showerror(
                "Error Fatal", f"Error crítico:\n{main_error}\nConsulte '{log_file}'."
            )
            root_err.destroy()
        except Exception as fallback_error:
            logger.error(
                f"No se pudo mostrar el mensaje de error fatal: {fallback_error}"
            )
