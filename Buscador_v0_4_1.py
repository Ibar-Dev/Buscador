...
class InterfazGrafica(tk.Tk):
    ...
    def _actualizar_tabla(self, tabla: ttk.Treeview, datos: Optional[pd.DataFrame], limite_filas: Optional[int] = None):
        try:
            ...
            print(f"Advertencia: Error Tcl al limpiar tabla: {e}")
        ...
        for i, (_, fila) in enumerate(datos_a_mostrar.iterrows()):
            ...
            try:
                ...
                print(f"Advertencia: Error Tcl al insertar fila {i}: {e}")
    ...
    def _cargar_diccionario(self):
        ...
        if self.motor.cargar_excel_diccionario(ruta):
            ...
            if df_dic_original is not None and not df_dic_original.empty:
                ...
                if len(df_dic_original.columns) > 0:
                    cols_a_mostrar_nombres.append(df_dic_original.columns[0])
                if len(df_dic_original.columns) > 3:
                    cols_a_mostrar_nombres.append(df_dic_original.columns[3])

                if cols_a_mostrar_nombres:
                    cols_validas = [c for c in cols_a_mostrar_nombres if c in df_dic_original.columns]
                    if cols_validas:
                        df_dic_display = df_dic_original[cols_validas].copy()
                    else:
                        print("Advertencia: Columnas 0 y/o 3 no encontradas para mostrar en vista previa.")
        ...
        if resultados is None:
            ...
            if num_res > 0:
                ...
                messagebox.showinfo("Sin Coincidencias Finales",
                                    f"Se encontraron términos en el diccionario para '{termino}', "
                                    "pero no se hallaron coincidencias finales en las descripciones.")
            ...
            if extension == 'csv':
                ...
                try:
                    ...
                    if len(self.resultados_actuales) > 65535:
                        messagebox.showwarning("Límite de Filas Excedido",
                                               f"El formato .xls solo soporta hasta 65,536 filas.\n"
