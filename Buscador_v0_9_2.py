...
class MotorBusqueda:
    ...
    def cargar_excel_diccionario(self, ruta_str: str) -> Tuple[bool, Optional[str]]:
        ...
        if df_cargado.shape[1] > 0: 
            ...
            max_cols_a_chequear_para_sinonimos = df_cargado.shape[1] 
    ...
    def _descomponer_nivel1_or(self, texto_complejo: str) -> Tuple[str, List[str]]:
        ...
        separadores_or = [(r"\s*\|\s*", "|")] # Solo "|" es OR de alto nivel
    ...
    def _parse_numero(self, num_str: Any) -> Optional[float]:
        ...
        if not isinstance(num_str, str): logger.debug(f"Parseo num: Entrada '{num_str}' no es string."); return None
        s_limpio = num_str.strip()
        if not s_limpio: logger.debug(f"Parseo num: Entrada '{num_str}' vacía tras limpiar."); return None
        
        logger.debug(f"Parseo num: Intentando convertir '{s_limpio}' (originado de '{num_str}')")
        try:
            if ',' not in s_limpio and '.' not in s_limpio: # Sin separadores
    ...
    def _generar_mascara_para_un_termino(self, df: pd.DataFrame, cols: List[str], term_an: Dict[str, Any], filtro_numerico_original: Optional[Dict[str, Any]] = None) -> pd.Series:
        tipo_termino = term_an["tipo"]; valor_termino = term_an["valor"]; unidad_requerida_canonica_query = term_an.get("unidad_busqueda")
        valor_a_comparar_final = valor_termino; unidad_final_para_comparar_canonica = unidad_requerida_canonica_query
                ...
                for indice_fila, valor_celda_raw in columna_serie.items():
                    ...
                    for match_num_unidad_celda in self.patron_num_unidad_df.finditer(texto_celda_str):
                        try:
                            ...
                            inicio_match_en_celda = match_num_unidad_celda.start(); fin_match_en_celda = match_num_unidad_celda.end()
                            char_antes_valido = (inicio_match_en_celda == 0) or (not texto_celda_str[inicio_match_en_celda - 1].isalnum())
                            char_despues_valido = (fin_match_en_celda == len(texto_celda_str)) or (not texto_celda_str[fin_match_en_celda].isalnum())
                            if not (char_antes_valido and char_despues_valido):
                                ...
                                continue
                            num_celda_str = match_num_unidad_celda.group(1); num_celda_val = self._parse_numero(num_celda_str)
                            unidad_celda_raw = match_num_unidad_celda.group(2)
                            ...
                            if not unidad_coincide: continue
                            ...
                            elif operador_final_para_comparar == "range" and ((valor_a_comparar_final[0] <= num_celda_val or np.isclose(num_celda_val, valor_a_comparar_final[0])) and (num_celda_val <= valor_a_comparar_final[1] or np.isclose(num_celda_val, valor_a_comparar_final[1]))): condicion_numerica_cumplida = True
                            if condicion_numerica_cumplida:
                                if filtro_numerico_original:
                                    texto_sinonimo_normalizado_de_fcd = self._normalizar_para_busqueda(term_an["original"])
                                    ...
                                    if re.search(patron_regex_sinonimo, self._normalizar_para_busqueda(texto_celda_str)):
                                        mascara_columna_actual_numerica.at[indice_fila] = True; break 
                                else: mascara_columna_actual_numerica.at[indice_fila] = True; break 
                        except ValueError: continue
                    if mascara_columna_actual_numerica.at[indice_fila]: break 
                mascara_total_termino |= mascara_columna_actual_numerica
            
            if tipo_termino == "str" and not filtro_numerico_original :
                try:
                    valor_normalizado_busqueda = str(valor_termino)
                    if not valor_normalizado_busqueda: continue
                ...
                except Exception as e: logger.warning(f"Error búsqueda STR en columna '{nombre_columna}' para término '{valor_termino}': {e}")
    ...
    def _combinar_mascaras_de_segmentos_or(self, lista_mascaras: List[pd.Series], df_idx_ref: Optional[pd.Index] = None) -> pd.Series:
        ...
        if idx_usar is None or idx_usar.empty:
            if lista_mascaras and not lista_mascaras[0].empty: idx_usar = lista_mascaras[0].index
        if idx_usar is None or idx_usar.empty: return pd.Series(dtype=bool)
    ...
    def _procesar_busqueda_en_df_objetivo(self, df_obj: pd.DataFrame, cols_obj: List[str], termino_busqueda_original_para_este_df: str, terminos_negativos_adicionales: Optional[List[str]] = None, return_mask_only: bool = False, filtro_numerico_original_desc: Optional[Dict] = None) -> Union[Tuple[pd.DataFrame, Optional[str]], Tuple[Optional[pd.Series], Optional[str]]]:
        ...
        if df_obj is None: df_obj = pd.DataFrame()
        df_despues_negaciones_query, terminos_positivos_de_query, _ = self._aplicar_negaciones_y_extraer_positivos(df_obj, cols_obj, termino_busqueda_original_para_este_df)
        ...
        if terminos_negativos_adicionales and not df_actual_procesando.empty:
            ...
            if query_solo_negativos_adicionales:
        ...
        if df_actual_procesando.empty and not terminos_positivos_final_para_parseo.strip():
            ...
            return (pd.Series(False, index=idx_ref_base) if return_mask_only else df_actual_procesando.copy()), None
        ...
        if not segmentos_nivel1_or:
            if termino_busqueda_original_para_este_df.strip() or terminos_positivos_final_para_parseo.strip():
                ...
                return (pd.Series(False, index=df_actual_procesando.index if not df_actual_procesando.empty else idx_ref_base) if return_mask_only else pd.DataFrame(columns=df_actual_procesando.columns)), msg_error_segmentos
            else:
        ...
        for segmento_or_actual in segmentos_nivel1_or:
            ...
            if not terminos_atomicos_analizados_and:
                if operador_nivel1 == "AND":
                    ...
                    return (pd.Series(False, index=df_actual_procesando.index if not df_actual_procesando.empty else idx_ref_base) if return_mask_only else pd.DataFrame(columns=df_actual_procesando.columns)), msg_error_and
                ...
                mascara_para_segmento_or_actual = self._aplicar_mascara_combinada_para_segmento_and(df_actual_procesando, cols_obj, terminos_atomicos_analizados_and, filtro_numerico_original_para_desc=filtro_numerico_original_desc)
        ...
        if not lista_mascaras_para_or and not df_actual_procesando.empty :
            ...
            return (pd.Series(False, index=idx_ref_base) if return_mask_only else pd.DataFrame(columns=df_obj.columns if df_obj is not None else [])), "Error interno: no se generaron máscaras OR."
        elif not lista_mascaras_para_or and df_actual_procesando.empty: return (pd.Series(dtype=bool) if return_mask_only else df_actual_procesando.copy()), None

        mascara_final_df_objetivo = self._combinar_mascaras_de_segmentos_or(lista_mascaras_para_or, df_actual_procesando.index if not df_actual_procesando.empty else idx_ref_base)
        if return_mask_only:
            ...
            elif not mascara_final_df_objetivo.any():
                 df_resultado_final = pd.DataFrame(columns=df_obj.columns if df_obj is not None else [])
            else: df_resultado_final = df_actual_procesando[mascara_final_df_objetivo].copy()
    ...
    def buscar(self, termino_busqueda_original: str, buscar_via_diccionario_flag: bool) -> Tuple[Optional[pd.DataFrame], OrigenResultados, Optional[pd.DataFrame], Optional[List[int]], Optional[str]]:
        ...
        if not termino_busqueda_original.strip():
            if self.datos_descripcion is not None: return self.datos_descripcion.copy(), OrigenResultados.DIRECTO_DESCRIPCION_VACIA, None, None, None
            else: return df_vacio_para_descripciones, OrigenResultados.DIRECTO_DESCRIPCION_VACIA, None, None, "Descripciones no cargadas."
        ...
        if terminos_positivos_globales.strip():
            ...
            if segs_l1:
                _op_l2, segs_l2 = self._descomponer_nivel2_and(segs_l1[0])
                if segs_l2:
                    terminos_analizados_temp = self._analizar_terminos([segs_l2[0]])
                    if terminos_analizados_temp and terminos_analizados_temp[0]["tipo"] in ["gt", "lt", "ge", "le", "eq", "range"] and terminos_analizados_temp[0].get("unidad_busqueda"):
        ...
        if buscar_via_diccionario_flag:
            ...
            if "+" in terminos_positivos_globales and not (terminos_positivos_globales.startswith('"') and terminos_positivos_globales.endswith('"')):
                ...
                fcds_obtenidos_final_para_ui = fcds_intento1
                ...
                if (fcds_obtenidos_final_para_ui is None or fcds_obtenidos_final_para_ui.empty) and \
                   ...
                   filtro_numerico_original_de_query.get("unidad_busqueda"):
                    ...
                    fcds_por_unidad, err_fcd_unidad = self._procesar_busqueda_en_df_objetivo(self.datos_diccionario, columnas_dic_para_fcds, query_solo_unidad_para_fcd, None)
                    if err_fcd_unidad: logger.warning(f"Error en búsqueda alternativa de FCDs por unidad '{query_solo_unidad_para_fcd}': {err_fcd_unidad}")
                    if fcds_por_unidad is not None and not fcds_por_unidad.empty:
                        ...
                        fcds_obtenidos_final_para_ui = fcds_por_unidad
                        indices_fcds_a_resaltar_en_preview = fcds_obtenidos_final_para_ui.index.tolist()
                        terminos_de_unidad_para_desc_set: Set[str] = set()
                        for _, fila_fcd_unidad in fcds_por_unidad.iterrows(): terminos_de_unidad_para_desc_set.update(self._extraer_terminos_de_fila_completa(fila_fcd_unidad))
                        ...
                        if not query_or_de_unidades_para_desc: return df_vacio_para_descripciones, OrigenResultados.VIA_DICCIONARIO_UNIDAD_SIN_RESULTADOS_DESC, fcds_obtenidos_final_para_ui, indices_fcds_a_resaltar_en_preview, "Query OR de unidades para descripciones (alternativa) vacía."
                        ...
                        resultados_desc_alt, error_desc_alt = self._procesar_busqueda_en_df_objetivo(self.datos_descripcion, columnas_desc_alt, query_or_de_unidades_para_desc, terminos_negativos_adicionales=neg_glob_alt, filtro_numerico_original_desc=filtro_numerico_original_de_query)
                        ...
                        if resultados_desc_alt is None or resultados_desc_alt.empty: return df_vacio_para_descripciones, OrigenResultados.VIA_DICCIONARIO_UNIDAD_SIN_RESULTADOS_DESC, fcds_obtenidos_final_para_ui, indices_fcds_a_resaltar_en_preview, None
                        else: return resultados_desc_alt, OrigenResultados.VIA_DICCIONARIO_UNIDAD_Y_NUMERICO_EN_DESC, fcds_obtenidos_final_para_ui, indices_fcds_a_resaltar_en_preview, None
                    else: 
                ...
                if fcds_obtenidos_final_para_ui is not None and not fcds_obtenidos_final_para_ui.empty: 
                    if indices_fcds_a_resaltar_en_preview is None: indices_fcds_a_resaltar_en_preview = fcds_obtenidos_final_para_ui.index.tolist()
...
# --- Interfaz Gráfica (El resto de la clase InterfazGrafica se mantiene igual que en v1.8.1) ---
# ... (copiar el resto de la clase InterfazGrafica y el bloque if __name__ == "__main__" de la respuesta anterior,
#      solo actualizando el número de versión en el título y el nombre del archivo de log a v1.10.3) ...
# --- (El resto del código de InterfazGrafica y __main__ es idéntico al de la v1.8.1,
#      solo cambia el título de la ventana y el nombre del archivo de log en __main__
#      a "v1.10.3 (Reconocimiento Magnitudes Mejorado)" y "Buscador_Avanzado_App_v1.10.3.log" respectivamente)
class InterfazGrafica(tk.Tk):
    ...
    def _ordenar_columna_tabla_ui(self,tabla,col,rev):
        ...
        if tabla==self.tabla_diccionario and self.motor.datos_diccionario is not None:
            columnas_para_diccionario_ordenado, _ = self.motor._obtener_nombres_columnas_busqueda_df(df_ord, self.motor.indices_columnas_busqueda_dic_preview, "diccionario_preview")
    ...
    def _ejecutar_busqueda_ui(self):
        ...
        if err_msg and origen.es_error_operacional:messagebox.showerror("Error Motor",f"Error interno: {err_msg}");self.resultados_actuales=pd.DataFrame(columns=df_desc_cols)
            ...
            if origen == OrigenResultados.VIA_DICCIONARIO_UNIDAD_SIN_RESULTADOS_DESC: msg_desc_i="pero no se encontraron coincidencias numéricas/de unidad en Desc."
            elif origen in [OrigenResultados.VIA_DICCIONARIO_SIN_TERMINOS_VALIDOS, OrigenResultados.VIA_DICCIONARIO_PURAMENTE_NEGATIVA_SIN_RESULTADOS_DESC]: msg_desc_i="pero no se extrajeron términos válidos para Desc."
            else: msg_desc_i="pero 0 resultados en Desc."
...
if __name__ == "__main__":
    LOG_FILE_NAME = "Buscador_Avanzado_App_v1.10.3.log" 
    logging.basicConfig(
        level=logging.DEBUG,
